...
const NextArrow = (props) => {
    ...
    return (
        <button
            onClick={onClick}
            className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-white rounded-full w-12 h-12 flex items-center justify-center shadow-xl z-10 -mr-6"
        >
...
const PrevArrow = (props) => {
    ...
    return (
        <button
            onClick={onClick}
            className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-white rounded-full w-12 h-12 flex items-center justify-center shadow-xl z-10 -ml-6"
        >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="w-5 h-5">
...
const Promotions = () => {
    ...
    return (
        <div className="w-full max-w-6xl mx-auto my-6 relative px-4 drop-shadow-xl">
            <Slider {...settings}>
                {promotions.map((promo, index) => (
                    <div key={index} className="px-2">
                        <div className="bg-white rounded-lg shadow-xl p-4 flex items-center h-24 bg-blend-normal	">
                            <img src={promo.img} alt="" className="mr-4 flex-shrink-0 h-12 w-12" />
